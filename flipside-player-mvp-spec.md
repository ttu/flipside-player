...
type QueueItem = { type: 'track'; spotifyUri: string; title: string; artist: string; albumId: string; albumArt?: string };
type QueueState = { items: QueueItem[] };

type DevicesState = { devices: { id: string; name: string; isActive: boolean }[]; loading: boolean };

type AuthState = { isAuthenticated: boolean; user?: { id: string; name: string; avatar?: string } };
```
...
const player = new Spotify.Player({
  ...
  getOAuthToken: cb => fetch('/spotify/token').then(r => r.text()).then(cb),
  volume: 0.8
});
...
### 7.3 Controls
```ts
await player.resume(); await player.pause();
await player.seek(ms);
await player.previousTrack(); await player.nextTrack();
